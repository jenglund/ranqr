...
- Matchup comparisons are normalized to always store the smaller item ID first
- When updating a vote, scores are adjusted by reversing the old result before applying the new one
- Tie votes don't change scores but are tracked for sub-score calculations
- Sub-scores are calculated recursively for items with the same main score
